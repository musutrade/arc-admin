...
## 当前主分支输入修正（2026-09-13）

PR #40 的主分支包含之前合入的 `chacha20 0.10.2` 锁文件更新，旧 `2faa1ca` 采集不能覆盖该工具输入。主机实际拒绝旧锁文件摘要后，已用公开 RC2 安装重新编译、采样三个原测试，并确认 1,778 个生产函数映射完整、两次二进制重新导出一致。Core 0.4.1 的签名配置采集、真实数值正负控制和重复采集拒绝均通过。

[系列变更回执](quality/20260913/backend-series-transition.json) 明确记录新旧锁文件和测量系列。此提交只采用新后端系列，不声明旧系列等价，不沿用旧后端基线；源码身份也由新原生结果重新生成。前端与 API 的输入摘要逐项匹配，保留原有测量。

待本配置进入主分支后，完整验收才可从包含新系列的真实 Git 提交建立首次基线。阈值、必需能力、完整执行步骤和正式 `cargo flow` 门禁均未改变。
