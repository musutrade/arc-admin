...
2. auditor 全量架构扫描；
3. backend：fmt、Clippy、check、临时 PostgreSQL、tests；
4. frontend：ESLint、Prettier、Vitest、production build；
5. workflow：arc-flow fmt、Clippy 和单元测试。
...
2. 由 arc-flow 启动并在结束时删除的 `postgres:16-alpine` 容器。
...
可用 `REPORT_DIR` 覆盖报告目录，用 `AUDITOR_CONFIG` 覆盖审计规则路径。
...
- Docker + 本地 `postgres:16-alpine`，或隔离的 `TEST_DATABASE_URL`。
